(function () {
    ...
    var deliver_no = Tools._GET().deliver_no || '';

    if(!deviceId || !deliver_no)
    {
        alert('缺少参数，请返回刷新重试');
        return;
    }
    ...
    common.checkLoginStatus(function () { //入口
        var deliver_no_t = Cookie.get("deliver_no");
        if(deliver_no_t == deliver_no){
            Tools.showAlert('该补货单已经提交过，请点击开门');
            $('.footer-goods').show();
            $('.btn-mini-cart').removeClass('disabled');
            $('.btn-add-cart').text("点击开门");
            open_scan();
        }else{
            getList();
        }
    ...
    $('.btn-add-cart').click(function (e) {
        ...
        if(deliver_no_t == deliver_no){
            ...
            Ajax.custom({
                url: config.API_CONFIRE_DELIVER,
                data: {
                    device_id: deviceId
                },
                type: 'POST',
                ...
                // $('.btn-add-cart').attr('order_name', response);
                $('.btn-add-cart').text("点击开门");
                Cookie.set("deliver_no", deliver_no, 0);
                open_scan();
            }, function (data) {
    ...
    function open_scan() {
        ...
        $('.btn-add-cart').text("开门中...");
        Ajax.custom({
            url: config.API_OPEN_SCAN,
            data: {
                device_id: deviceId
            },
            type: 'POST',
