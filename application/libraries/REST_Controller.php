...
abstract class REST_Controller extends \CI_Controller {
    ...
    const USER_NO_LOGIN = 444;//自定义，用户未登录
    // Server Error
    ...
    {
        parent::__construct();
        header('Access-Control-Allow-Headers:Content-Type,Authorization,Content-Length,X-Requested-With,Accept,x-csrf-token,origin,token');
     ...
     */
    public function response($data = NULL, $http_code = NULL,$not_die = 0)
    {
//        ob_start();
...
//
    ...
    {
        if($this->uri->uri_string() != "api/device/receive_heart_msg" && $this->uri->uri_string() != "api/device/receive_heart_new_msg") {
    ...
    public function raw_data($key = NULL, $xss_clean = NULL)
    {
        $raw_data = NULL;
        if($GLOBALS['HTTP_RAW_POST_DATA']){
            $raw_data = json_decode($GLOBALS['HTTP_RAW_POST_DATA'],true);
        }else{
        ...
        return isset($raw_data[$key]) ? $this->_xss_clean($raw_data[$key], $xss_clean) : NULL;
    }
    /**
    ...
    protected function _check_php_session()
    {
        ...
        $key = $this->config->item('user_info');
     ...
     * 用户自定义授权规则
     */
    protected function _check_custom_auth()
    {
        $platform = $this->input->get_request_header('Platform', TRUE);
//        write_log('platform:'.$platform);
        $token = $this->get_wap_token();
        if(!$platform){
            $platform = "wap";//默认wap来源
        }else if(in_array($platform,array("device","admin","client","bjdevice"))){
            $token = md5("token".rand(1,99999));
        }else if($platform == "wap"){
            //wap请求检查 Referer 防止跨域攻击
            $refer = $this->_head_args['Referer'];
            $refer_host  = parse_url($refer);
            $allow_host = $this->config->item('allow_host');
            if(!in_array($refer_host['host'],$allow_host)){
                $this->response([
                    $this->config->item('rest_status_field_name') => FALSE,
                    $this->config->item('rest_message_field_name') => '非法请求'
        ...
        //不验证授权uri
        $no_auth_uri = $this->config->item('no_auth_uri');
        if(!in_array(strtolower($this->uri->uri_string()),$no_auth_uri)){

//            $token = $this->input->get_request_header('Token', TRUE);
            $sign = $this->input->get_request_header('Sign', TRUE);
//            write_log('sign:'.$sign);
            $all_uris = $this->config->item($platform.'_auth_uri');
            $uri = $this->uri->uri_string();

//            echo $sign,',',$this->_check_sign($platform),!in_array($uri,$all_uris);die;
            if(empty($token)){
                $this->response([
                    $this->config->item('rest_status_field_name') => FALSE,
                    $this->config->item('rest_message_field_name') => 'token缺失'
                ], self::HTTP_UNAUTHORIZED);
            }
            if(empty($sign)){
                $this->response([
                    $this->config->item('rest_status_field_name') => FALSE,
                    $this->config->item('rest_message_field_name') => 'sign缺失'
                ], self::HTTP_BAD_REQUEST);
            }
            if($sign != $this->_check_sign($platform)){
                $this->response([
                    $this->config->item('rest_status_field_name') => FALSE,
                    $this->config->item('rest_message_field_name') => '签名错误'
                ], self::HTTP_BAD_REQUEST);
            }
            if(!in_array(strtolower($uri),$all_uris)){
                $this->response([
                    $this->config->item('rest_status_field_name') => FALSE,
                    $this->config->item('rest_message_field_name') => 'URL未授权'
                ], self::HTTP_BAD_REQUEST);
            }
        }
    }

    public function get_wap_token($sess_cookie_name='token'){
        return $this->input->get_request_header(token, TRUE);
//        if(isset($this->_head_args['Cookie'])){
//            $cookie_str = $this->_head_args['Cookie'];
//            if(strlen($cookie_str)<=0){
//                return "";
//            }
//            $cookie_arr = explode(";",$cookie_str);
//            foreach($cookie_arr as $v){
//                if(check_str_exist($v,$sess_cookie_name)){
//                    return substr($v,strlen($sess_cookie_name)+1);
//                }
//            }
//        }else if($this->input->get_request_header($sess_cookie_name, TRUE)){
//            return $this->input->get_request_header($sess_cookie_name, TRUE);
//        }
//        return "";
    }
    /**
     ...
     * @return string
     */
    private function _check_sign($platform = "wap"){
        if(in_array($platform,array('device','admin','client','bjdevice','wap'))){
            if(in_array($platform,array('device','bjdevice'))){
                $params = $this->raw_data();
            }else{
                $params = $this->_args;
                if($this->request->method=="get"){
                    $params = $this->_get_args;
            ...
            ksort($params);
            $query = '';
            $api_secrets = $this->config->item('api_secret');
            $api_secret = $api_secrets[$platform];
            ...
            write_log('query+secret=>'.$query.$api_secret);
            write_log('query+secret+md5=>'.md5($query.$api_secret));
            write_log('query+secret+md5-末位+salt=>'.substr(md5($query.$api_secret), 0,-1).$salt);
            write_log('query+secret+md5-末位+salt+md5=>'.md5(substr(md5($query.$api_secret), 0,-1).$salt));
            */
            return md5(substr(md5($query.$api_secret), 0,-1).$salt);
        }
        return '';
    }
    ...
    function send_error_response($message,$status = FALSE)
    {
        $this->response([
            'status' => $status,
            'message' => $message
    ...
    function send_no_login_response($message)
    {

    }


    public function send_ok_response($data = NULL, $http_code = NULL,$not_die = 0)
    {
        if(!$http_code)
        {
            $this->response($data, REST_Controller::HTTP_OK, $not_die);
        }else
        {
            $this->response($data, $http_code, $not_die);
        }
    }
     ...
     * @return Array
     */
    public function get_curr_user(){
        $token = $this->get_wap_token();
        $user_cache_key = 'user_'.ltrim($token,'=');
        $user = $this->cache->get($user_cache_key);
        if(!$user OR !is_array($user) OR !isset($user['id']) OR empty($user['id'])){
            $this->response([
                'status' => FALSE,
                'message' => "用户信息异常"
            ], self::HTTP_UNAUTHORIZED);
    ...
    {
        if (null_para($data)) {
            $this->send_error_response($msg);
        }
    }

    public function check_mobile_and_send_error($mobile, $msg)
    {
        if (!is_mobile($mobile)) {
            $this->send_error_response($msg);
        }
