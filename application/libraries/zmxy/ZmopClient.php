...
class ZmopClient {
	...
	public function execute($request) {
		...
		$sysParams[$this->signParam] = RSAUtil::sign($apiParamsQuery, $this->privateKey);


		//加密,只对业务参数进行加密
		$encryptedApiParams[$this->bizParam] = RSAUtil::rsaEncrypt($apiParamsQuery, $this->zhiMaPublicKey);
		...
		if ("json" == $this->format) {
			...
			if($encrypted){
				$biz_response = RSAUtil::rsaDecrypt($biz_response, $this->privateKey);
			}
        ...
        if(isset($respObject->biz_response_sign)){
            ...
            $verifyResult = RSAUtil::verify($biz_response, $bizResponseSign, $this->zhiMaPublicKey);
		...
		if (isset ($bizRespObject->error_code)) {
            $this->logCommunicationError($sysParams["method"], $requestUrl, "HTTP_ERROR_" . $bizRespObject->error_code, $bizRespObject->error_message);
        }
	...
	public function generatePageRedirectInvokeUrl($request){
		...
		$systemParams[$this->signParam] = RSAUtil::sign($apiParamsQuery, $this->privateKey);
		...
		$bizParam =  RSAUtil::rsaEncrypt($apiParamsQuery, $this->zhiMaPublicKey);
	...
	public function decryptAndVerifySign($encryptedResponse, $sign){
		...
		$decryptedResponse = RSAUtil::rsaDecrypt($encryptedResponse, $this->privateKey);
		$success = RSAUtil::verify($decryptedResponse, $sign, $this->zhiMaPublicKey);
		if($success){
			return strstr ( $decryptedResponse, '%' ) ? urldecode ($decryptedResponse ) : $decryptedResponse;
		}else{
	...
	public function generateSign($request){
		...
		return RSAUtil::sign($paramStr, $this->privateKey);
	}
	...
	public function generateEncryptedParam($request){
		$paramStr = $this->getBizParamStr($request);
		return RSAUtil::rsaEncrypt($paramStr, $this->zhiMaPublicKey);
	}
